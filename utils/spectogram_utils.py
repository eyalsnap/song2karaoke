...
class WavConverter():
    ...
    def wav2spectogram(self, data):
        ...
        mel_spec = make_mel(wav_spectrogram, self.mel_filter, shorten_factor=shorten_factor)
        return mel_spec

    def spectogram2wav(self, spec):
        ...
        inverted_mel_audio = inverted_mel_audio / np.max(np.abs(inverted_mel_audio))
        return inverted_mel_audio


if __name__ == '__main__':
    mywav = r"C:/dev/free_time_projects/kareoke/sample_data/_aiti_loi__zz_btirof_/karaoke.wav"
    out_path = r'C:\temp\outwav.wav'
    rate, data = wavfile.read(mywav)
    converter = WavConverter()
    data = data[:, 0]
    data = data[rate * 20: rate * 30]
    spec = converter.wav2spectogram(data)
    reconstructed = converter.spectogram2wav(spec)
    sd.play(data, rate,blocking=True)
    sd.play(reconstructed, rate,blocking=True)
    scipy.io.wavfile.write(out_path,rate,reconstructed)
